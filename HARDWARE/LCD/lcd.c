...
{
	if(LCD_Obj.Date.Hour != Date->Hour)
	{
		
		if(LCD_Obj.Date.Hour/10 != Date->Hour/10)
			LCD_ShowNum(100, 100, (u32)Date->Hour/10, 1, 24);
		if(LCD_Obj.Date.Hour%10 != Date->Hour%10)
			LCD_ShowNum(125, 100, (u32)Date->Hour%10, 1, 24);
		
		LCD_Obj.Date.Hour = Date->Hour;
	}
	
	if(LCD_Obj.Date.Min != Date->Min)
	{
		
		if(LCD_Obj.Date.Min/10 != Date->Min/10)
			LCD_ShowNum(175, 100, (u32)Date->Min/10, 1, 24);
		if(LCD_Obj.Date.Min%10 != Date->Min%10)
			LCD_ShowNum(200, 100, (u32)Date->Min%10, 1, 24);
		
		LCD_Obj.Date.Min = Date->Min;
	}
	
	if(LCD_Obj.Date.Sec != Date->Sec)
	{
		
		if(LCD_Obj.Date.Sec/10 != Date->Sec/10)
			LCD_ShowNum(225, 100, (u32)Date->Sec/10, 1, 16);
		if(LCD_Obj.Date.Sec%10 != Date->Sec%10)
			LCD_ShowNum(242, 100, (u32)Date->Sec%10, 1, 16);
		
		LCD_Obj.Date.Sec = Date->Sec;
	}
